...
def reset_disabled_portals() -> list[str]:
    """Re-enable all auto-disabled portals (e.g. after scraper fixes)."""
    data = _load()
    previously = list(data.get("disabled", []))
    data["disabled"] = []
    for entry in data.get("portals", {}).values():
        if entry.get("zero_streak", 0) >= config.PORTAL_ZERO_STREAK_DISABLE:
            entry["zero_streak"] = 0
    _save(data)
    return previously
